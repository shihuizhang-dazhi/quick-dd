...
try:
    from port_scan import scan_hosts as port_scan_hosts, scan_host as port_scan_host
except Exception:
    port_scan_host = None
    port_scan_hosts = None
...
def main() -> None:
    ...
    def _run_port_scan(alive_hosts_list):
        """对存活主机进行端口扫描"""
        if port_scan_hosts is not None:
            try:
                output = port_scan_hosts(alive_hosts_list)
                return {h: ports for h, (_, ports, _) in output.items() if ports}
            except Exception:
                pass
