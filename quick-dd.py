...
try:
    from port_scan import scan_host as port_scan_host, COMMON_PORTS
except Exception:
    COMMON_PORTS = [80, 443, 22, 21, 25, 3306, 8080, 8443, 8888, 9000]
    def port_scan_host(host, mode="tcp"):
        return host, []
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Quick-DD 资产发现工具")
    parser.add_argument("domain", help="目标域名")
    parser.add_argument("--fofa", action="store_true", help="启用 FOFA 查询")
    parser.add_argument("--port-scan", action="store_true", help="启用端口扫描")
    parser.add_argument("--fofa-query", default="", help="自定义 FOFA 查询语句")
    parser.add_argument("--fofa-size", type=int, default=100, help="FOFA 查询条数（默认 100）")
    parser.add_argument("--ports", default="", help="自定义端口扫描列表，支持范围如 1-1024 或逗号分隔如 80,443,8080")
