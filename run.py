...
输出: 一个三 sheet 的 Excel 文件
  1. assets      - 正常资产结果
  2. alive_ips   - chapangzhan 历史发现的存活 IP
  3. ip_domains  - rdnsdb 的 IP 对应域名
"""
...
def main():
    ...
    ip_domain_rows = []
    if scan_cidr:
        print(f"  {DIM}| chapangzhan...{R}", end=" ", flush=True)
        try:
            alive_ips = get_live_ips_from_chapangzhan(scan_cidr)
            print(f"{G}{len(alive_ips)}{R}", end="")
        except Exception:
            print(f"{RED}?{R}", end="")
    ...
    ws2 = wb.create_sheet("alive_ips")
    ws2.append(["IP", "来源"])
    for ip in alive_ips:
        ws2.append([ip, "历史发现"])

    ws3 = wb.create_sheet("ip_domains")
    ws3.append(["IP", "域名", "来源"])
    for ip, domain_name in ip_domain_rows:
        ws3.append([ip, domain_name, "rdnsdb"])
    ...
    try:
        alive_ips_file = os.path.join(output_dir, f"{base_domain}_alive_ips.txt")
        with open(alive_ips_file, "w", encoding="utf-8") as af:
            for ip in alive_ips:
                af.write(ip + "\n")
    except Exception:
        pass
