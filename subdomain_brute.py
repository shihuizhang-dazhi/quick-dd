...
async def wildcard_test(domain, dns_servers, count=3):
    ...
    返回: (is_wildcard, wildcard_ips_set, wildcard_content_hash)
    """
    random_suffixes = []
    ...
    if resolved_count >= count - 1 and all_ips:
        # 获取泛解析页面的内容 hash，用于后续 HTTP 二次验证
        wildcard_hash = _fetch_wildcard_hash(random_suffixes[0])
        return True, all_ips, wildcard_hash

    return False, set(), None


def _fetch_wildcard_hash(hostname):
    """获取泛解析页面的 HTTP 内容 hash，用于后续对比"""
    import hashlib
    from urllib.request import Request, urlopen
    for proto in ("http", "https"):
        try:
            req = Request(f"{proto}://{hostname}/",
...
def _http_content_differs(hostname, wildcard_hash):
    """HTTP 请求对比，页面内容与泛解析页面不同则返回 True（认为是真实子域名）"""
    import hashlib
    from urllib.request import Request, urlopen
    if not wildcard_hash:
        return False
    for proto in ("http", "https"):
        try:
            req = Request(f"{proto}://{hostname}/",
...
async def batch_dns_resolve(hostnames, dns_servers, concurrency=MAX_CONCURRENCY,
    ...
    async def resolve_and_filter(hostname):
        ...
        if ips and wildcard_ips:
            non_wildcard = [ip for ip in ips if ip not in wildcard_ips]

            if not non_wildcard:
                # 所有 IP 都在泛解析黑名单 → HTTP 二次验证
                if wildcard_hash and _http_content_differs(hostname, wildcard_hash):
                    # 页面内容与泛解析不同 → 真实子域名，保留
                    pass
                else:
                    return  # 确认是泛解析，丢弃
...
def main():
    ...
    if not args.no_wildcard:
        print(f"\n{BOLD}[2/6] 泛解析检测{R}")
        is_wildcard, wildcard_ips, wildcard_hash = asyncio.run(wildcard_test(domain, dns_servers))
        if is_wildcard:
            if args.no_filter:
                ...
                print(f"  {Y}检测到泛解析 -> IP: {', '.join(sorted(wildcard_ips)[:3])}...{R}")
                if wildcard_hash:
                    print(f"  {G}已开启泛解析过滤（DNS + HTTP 双重验证）{R}")
                else:
                    print(f"  {G}已开启泛解析过滤（仅 DNS，未获取到页面 hash）{R}")
        else:
            print(f"  {G}未检测到泛解析{R}")
    else:
    ...
    resolved = asyncio.run(
        batch_dns_resolve(
            ...
            wildcard_hash=wildcard_hash if not args.no_filter else None,
