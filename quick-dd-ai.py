        ...
        if icp_domains:
            ...
            def _check_resolve(domain):
                ...
                try:
                    ...
                    pass
                # 方法2: HTTP HEAD 请求（某些域名 DNS 不通但 HTTP 可达）
                try:
                    requests.head(f"http://{domain}", timeout=8, allow_redirects=False)
                    return domain, True
                except Exception:
                    pass
                # 方法3: HTTPS HEAD 请求
                try:
                    requests.head(f"https://{domain}", timeout=8, allow_redirects=False, verify=False)
                    return domain, True
                except Exception:
                    return domain, False
            ...
            # 交互式选择（仅在非 --yes 模式且有不可达域名时）
            if dead_domains and not args.yes and not args.only_domains and not args.dns_only:
                ...
                print(f"\n  {BOLD}选择要扫描的域名:{R}")
                print(f"    {G}[A]{R} 全部域名 ({len(all_domains)} 个，含不可达的)")
                print(f"    {G}[D]{R} 仅可达域名 ({len(alive_domains)} 个)")
                print(f"    {DIM}[数字]{R} 手动输入域名编号（用逗号分隔，如 1,3,5）")
                try:
                    choice = input(f"  {DIM}选择 [A/d/编号]:{R} ").strip().lower()
