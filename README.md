...
| 📋 **备案查域名** | 通过 ICP 备案  ，按主体名称批量查备案域名 |
| 🔍 **多源子域采集** | ip138 API + CRT 证书日志 + 字典爆破 + FOFA，四源融合 |
| 🛡️ **泛解析检测** | 自动检测并过滤泛解析 IP，避免大量虚假结果 |
| ⚡ **异步高并发** | 500 并发 UDP DNS 查询 + 异步 HTTP 探活 |
| 🔌 **端口扫描** | 两阶段 TCP 扫描（快速检测 + 服务识别），支持 30+ 服务指纹 |
| 📊 **报告输出** | Excel 多 Sheet 报告 + URLs.txt（可直接喂给 nuclei/dddd） |
...
输入名称（如"山东大学"）
  ...
  3. 📋 天眼查/百度商机宝 → 查该主体所有备案域名
  4. 🧠 智谱 AI 分析企业关系（本部/下属/无关）
  5. ✂️ 精确过滤（只保留完全匹配的域名）
  6. 🔎 DNS 解析检查 + 交互式选择
  │
  ▼ 对每个域名执行 quick-dd.py
  7. 🔍 四源子域采集（ip138 + 字典 + CRT + FOFA）
  8. ⚡ 并发探活（HTTP/HTTPS）
  9. 🔌 端口扫描（默认 Top100）
  10. 🌐 IP/ASN/反向DNS 查询
  │
...
天眼查通过 `tianyancha_request.txt` 文件中的 Cookie 进行认证：

1. 打开 https://beian.tianyancha.com 并登录
2. F12 → Network → 搜索任意公司名
3. 找到备案查询请求，复制完整 HTTP 请求
4. 粘贴到项目目录下的 `tianyancha_request.txt` 文件中
