...
domain.py                # 子域采集 + 探活
subdomain_brute.py       # 字典爆破（异步DNS + 泛解析检测 + 递归爆破）
...
| ② 泛解析检测 | 随机子域名测试，检测并过滤泛解析 IP，避免大量误报 |
| ③ 字典爆破 | 异步 UDP DNS 查询（500 并发），加载 subnames.txt (15K) + subnames_full.txt (76K) 字典 |
| ④ 二层递归爆破 | 对一级发现的子域名用 next_sub_full.txt (1330 词) 字典递归爆破下一级 |
| ⑤ CT 日志补全 | 通过 crt.sh 查询证书透明度日志 |
| ⑥ 并发探活 | HTTP(80) / HTTPS(443) / HTTP(8080) / HTTPS(8080) |
| ⑦ 端口扫描 | TCP connect 两阶段扫描（快速检测 + 服务识别），带二次验证减少误报 |
...
## subdomain_brute.py 用法

子域名爆破工具，可独立运行或被 `quick-dd.py` 调用。

### 基本用法

```bash
# 默认模式（subnames.txt 15K 词，快速）
python subdomain_brute.py example.com

# 完整字典（subnames_full.txt 76K 词，覆盖广）
python subdomain_brute.py example.com --full

# 仅 DNS 解析，不探活
python subdomain_brute.py example.com --no-probe

# 指定并发数
python subdomain_brute.py example.com --concurrency 1000

# 跳过泛解析检测
python subdomain_brute.py example.com --no-wildcard

# 禁用下一级递归爆破
python subdomain_brute.py example.com --no-next-level

# 指定字典文件
python subdomain_brute.py example.com -d subnames.txt my_dict.txt
```
...
|------|------|
| `domain` | 目标域名 |
| `-d, --dict` | 指定字典文件（默认使用 subnames.txt + subnames_full.txt） |
| `--full` | 使用完整字典 subnames_full.txt（76K 词） |
| `--no-next-level` | 禁用下一级递归爆破 |
| `--no-probe` | 跳过 HTTP 探活，仅做 DNS 解析 |
...
| 特性 | 说明 |
|------|------|
| **泛解析检测** | 解析 3 个随机子域名，检测域名是否配置了泛解析（`*.example.com → IP`），自动过滤泛解析 IP，避免大量虚假结果 |
| **异步 UDP DNS** | 直接构造 DNS 查询包发送 UDP，500 协程并发，绕过系统 DNS 缓存 |
| **多 DNS 轮询** | 腾讯 119.29.29.29、阿里 223.5.5.5 等多 DNS 服务器轮询，避免单 DNS 限速 |
| **二层递归** | 对一级发现的子域名用 1330 个前缀递归爆破下一级（如 `admin.web.xxx.com`） |
| **专用线程池** | 500 线程池替代 Python 默认的 32 线程池，真正实现高并发 |

### 字典文件

`brute_dict/` 目录下的字典文件（来自 [subDomainsBrute](https://github.com/lijiejie/subDomainsBrute)）：

| 文件 | 词数 | 说明 |
|------|------|------|
| `subnames.txt` | 15,375 | **默认字典**，覆盖常见子域名 |
| `subnames_full.txt` | 76,119 | 完整字典，覆盖面更广（`--full` 或 quick-dd 默认加载） |
| `next_sub.txt` | 163 | 下一级递归字典（快速模式） |
| `next_sub_full.txt` | 1,330 | 下一级递归字典（默认使用） |
...
| 阶段 | 说明 |
|------|------|
| 阶段1：快速 TCP Connect | 并发检测端口是否开放，带**二次验证**：连接成功后等待 0.5s 检查连接是否被 RST，过滤防火墙/负载均衡器假响应 |
| 阶段2：服务识别 | 仅对确认开放的端口做 Banner 抓取（SSH/FTP/HTTP/数据库等） |

### 自适应扫描策略

1. 先扫描 Top20 高频端口（80, 443, 8080 等）
2. 如果高频端口全部不通 → 用剩余端口前 10 个探活
3. 仍然全部不通 → 跳过该主机，节省时间
4. 主机可达 → 继续扫描剩余端口
...
```bash
python port_scan.py urls.txt          # TCP connect 扫描
python port_scan.py --syn urls.txt    # SYN 半连接扫描（需 root）
python port_scan.py --tcp urls.txt    # 仅 TCP connect
```

### 支持的服务识别

SSH (22)、FTP (21)、SMTP (25)、HTTP/HTTPS (80/443/8080 等)、MySQL (3306)、PostgreSQL (5432)、Redis (6379)、MongoDB (27017)、MSSQL (1433)、Oracle (1521)、Memcached (11211)、Elasticsearch (9200)
