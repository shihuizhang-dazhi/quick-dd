...
DEFAULT_SIZE = 100  # 每次查询条数（免费版最大 100）
MAX_PAGES = 10      # 最多翻页数
...
def _query(q: str, email: str, key: str, size: int = DEFAULT_SIZE, page: int = 1) -> List[List[str]]:
...
def search(q: str, size: int = DEFAULT_SIZE) -> List[Dict]:
    ...
    if not conf:
        ...
        print(f"      1. 创建 {CONF_FILE}，内容: {{\"email\":\"xxx\",\"key\":\"xxx\"}}")
        print(f"      2. 或设置环境变量 FOFA_EMAIL 和 FOFA_KEY")
        return []
    ...
    for page in range(1, pages + 1):
        batch = _query(q, conf["email"], conf["key"],
